...
// MARK: - Animal Tag Helper Model (for dropdown picker)
struct AnimalTag: Identifiable {
    let id: Int
    let name: String
    let tag: String
    var displayLabel: String { "\(name) – \(tag)" }
}
...
struct NewMilkEntryView: View {
    ...
    // Animal guard + dropdown state
    @State private var animalTags: [AnimalTag] = []
    @State private var isLoadingAnimals = true
    ...
    /// True when the user owns at least one animal
    private var hasAnimals: Bool { !animalTags.isEmpty }
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            VStack(spacing: 0) {
                ...
                if isLoadingAnimals {
                    // Loading state
                    Spacer()
                    ProgressView("Loading your animals…")
                        .tint(.cyan)
                    Spacer()
                    ...
                    ScrollView(showsIndicators: false) {
                        VStack(spacing: 20) {
                            ...
                            // Cattle Tag – dropdown picker (for both Bulk & Individual Milk)
                            outlinedField(label: "Cattle Tag No.", icon: "person.text.rectangle", isRequired: entry.milkType == .individual) {
                                Button {
                                    showTagPicker = true
                                } label: {
                                    HStack {
                                        Text(entry.cattleTag.isEmpty ? (entry.milkType == .individual ? "Select an animal…" : "Select animal (optional)") : entry.cattleTag)
                                            .font(.system(size: 16))
                                            .foregroundColor(entry.cattleTag.isEmpty ? Color(.placeholderText) : .primary)
                                        Spacer()
                            ...
                            // Total Milk Product (computed, read-only)
                            ...
                            // Cow Milked Number (Bulk milk, or when 'All' animals selected)
                            if entry.milkType == .bulk || entry.cattleTag == "All" {
                                outlinedField(label: "Cow Milked number", icon: "number.circle") {
            ...
            // ── Save Button (only when animals exist) ──────────────────
            if hasAnimals && !isLoadingAnimals {
                VStack {
                    Spacer()
                    Button {
        ...
        .onAppear {
            if let existing = existingEntry { self.entry = existing }
            loadAnimals()
        }
        .sheet(isPresented: $showTagPicker) {
            AnimalTagPickerSheet(
                tags: animalTags,
                selectedTag: $entry.cattleTag,
                onSelectAll: {
                    entry.cattleTag = "All"
                    entry.cowMilkedNumber = animalTags.count
                }
            )
        }
    }

    // MARK: - No Animals Empty State
    private var noAnimalsView: some View {
        VStack(spacing: 24) {
            Spacer()
            ...
            Text("No Animals Added")
                .font(.system(size: 22, weight: .bold))

            Text("You need to add at least one animal\nbefore recording milk production.")
                .font(.system(size: 15))
    ...
    // MARK: - Fetch Animal Tags
    private func loadAnimals() {
        isLoadingAnimals = true
        NetworkManager.shared.getAnimalTags { rawList in
            self.animalTags = rawList.compactMap { dict -> AnimalTag? in
                guard let id  = dict["id"]   as? Int,
                      let nm  = dict["name"] as? String,
                      let tg  = dict["tag"]  as? String else { return nil }
                return AnimalTag(id: id, name: nm, tag: tg)
            }
            self.isLoadingAnimals = false
        }
    }
...
// MARK: - Animal Tag Picker Sheet

struct AnimalTagPickerSheet: View {
    let tags: [AnimalTag]
    ...
    var body: some View {
        NavigationStack {
            List {
                // Individual animals
                ForEach(tags) { animal in
                    Button {
                        selectedTag = animal.tag
                        dismiss()
                    } label: {
                        HStack {
                            VStack(alignment: .leading, spacing: 4) {
                ...
                // All Animals option at the bottom
                Button {
                    onSelectAll?()
                    dismiss()
                } label: {
                    HStack {
                        VStack(alignment: .leading, spacing: 4) {
                            Text("All")
                                .font(.system(size: 16, weight: .semibold))
                                .foregroundColor(.primary)
                            Text("All \(tags.count) animals – count auto-filled")
                                .font(.system(size: 13))
                                .foregroundColor(.secondary)
                        }
                        Spacer()
...
#Preview {
    NavigationStack {
        MilkRecordListView()
            ...
            .environmentObject(MilkDataManager())
    }
}
