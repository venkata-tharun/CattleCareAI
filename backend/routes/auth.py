...
def register():
    ...
    if not full_name or not email_or_phone or not password:
        return jsonify({"error": "full_name, email_or_phone and password are required"}), 400
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Delete any previous OTPs for this context/contact
            cur.execute("DELETE FROM otp_codes WHERE email_or_phone = %s AND context = %s", (email_or_phone, 'registration'))
            
            # Store OTP
            cur.execute(
                "INSERT INTO otp_codes (email_or_phone, otp_code, context, expires_at) VALUES (%s, %s, %s, %s)",
                (email_or_phone, otp_code, 'registration', expires_at)
            )
            
            # Store registration data temporarily in session (since we don't have a pending_users table)
            session[f"reg_{email_or_phone}"] = {
                "full_name": full_name,
                "farm_name": farm_name,
            ...
            # Send OTP via Email 
            # (Assuming email_or_phone is an email address when SMTP is used)
            from app import mail
            success = send_otp_email(mail, email_or_phone, otp_code, 'registration')
            if not success:
...
def verify_registration():
    ...
    try:
        with conn.cursor() as cur:
            # Check OTP
            cur.execute(
                "SELECT * FROM otp_codes WHERE email_or_phone = %s AND context = 'registration' AND otp_code = %s ORDER BY id DESC LIMIT 1",
            ...
            # OTP is valid, retrieve pending user data
            reg_data = session.get(f"reg_{email_or_phone}")
            if not reg_data:
            ...
            cur.execute(
                ...
                (reg_data["full_name"], email_or_phone, reg_data["farm_name"], reg_data["password_hash"])
            )
            
            # Cleanup OTP and Session
            cur.execute("DELETE FROM otp_codes WHERE email_or_phone = %s AND context = 'registration'", (email_or_phone,))
            session.pop(f"reg_{email_or_phone}", None)
